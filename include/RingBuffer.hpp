...
class RingBuffer {
  public:
    ...
    RingBuffer() : mStart(0), mCapacity(0), mSize(0) { }
    
    /** \brief initialize a buffer of capacity c
     * \param capacity of the buffer **/
    RingBuffer(uint32_t c) : mStart(0), mCapacity(c), mSize(0), mData(c) { }
    ...
    /** \brief is the buffer empty or not **/
    bool empty() {
        return (mSize == 0);
    }

    void reserve(uint32_t c) {
        mStart = 0;
        mSize = 0;
        mCapacity = c;
        mData.reserve(c+c/2);
